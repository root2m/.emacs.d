...
** Utils
#+BEGIN_SRC emacs-lisp
  (defmacro lmm/define-key-remap (map from-map &rest keys)
    "(lmm/define-key-remap setmap from-map \"C-n\" command ...)
    => (define-key setmap [remap from-map-C-n-command] #'command)"
    (declare (indent 1))
    (let ((num 1)
          li
          ocommd)
      (dolist (km keys)
        (if (= (% num 2) 0)
            (push `(define-key ,map [remap ,ocommd] ,km) li)
          (setq ocommd (lookup-key (eval from-map) (kbd km))))
        (setq num (1+ num)))
      `(progn ,@li)))

  (defvar lmm/key-dispath-overlay (make-overlay 0 0))
  (defun lmm/key-dispath-command-def (callback delay put-overlay &rest keybinds)
  ...
  non nil display default `lmm/key-dispath-overlay'.
  KEYBINDS: is binds."
    (lambda ()(interactive)
      (let* ((this-key (this-command-keys))
             (p (point))
             (overlay
              (cond ((functionp put-overlay)
                     (funcall put-overlay))
                    ((null put-overlay) nil)
                    ((and (eobp) (= (current-column) 0))
                     (overlay-put lmm/key-dispath-overlay 'display (concat "\n" this-key))
                     (move-overlay lmm/key-dispath-overlay (1- p) p))
                    ((eolp)
                     (overlay-put lmm/key-dispath-overlay 'display (concat this-key "\n"))
                     (move-overlay lmm/key-dispath-overlay p (1+ p)))
                    (t
                     (overlay-put lmm/key-dispath-overlay 'display this-key)
                     (move-overlay lmm/key-dispath-overlay p (1+ p)))))
             (evt (read-event nil nil delay))
             command)
        (and overlay (delete-overlay overlay))
        (cond ((and (characterp evt)
                    (setq command (plist-get keybinds (char-to-string evt) 'equal)))
...
#+BEGIN_SRC emacs-lisp
  (use-package evil
    ...
    (evil-define-key* 'insert 'global
      "j" (lmm/key-dispath-command-def 'self-insert-command 0.2 t
                                       "k" 'evil-normal-state
                                       "l" 'toggle-input-method
                                       "w" 'forward-to-word
                                       "b" 'backward-to-word
...
#+BEGIN_SRC emacs-lisp
  (use-package consult
    ...
    ;; (defvar consult-toggle-search-current-command nil)
    ;; (defvar consult-toggle-search-initial-string nil)

    ;; (defun consult-toggle-search (&optional action default-text async)
    ;;   "search in current buffer or current directory
    ;;     -> search in all buffer or up directory."
    ;;   (interactive)
    ;;   (cond
    ;;    ((and (minibufferp) (equal this-command 'consult-toggle-search))
    ;;     (let* ((string (buffer-substring-no-properties (minibuffer-prompt-end)
    ;;                                                    (point-max)))
    ;;            args)
    ;;       (pcase consult-toggle-search-current-command
    ;;         ('consult-line
    ;;          (setq args '(consult-line-multi nil)))

    ;;         ('consult-imenu
    ;;          (setq args '(consult-imenu-multi)))

    ;;         ((or 'consult-ripgrep 'consult-find 'consult-grep)
    ;;          (when-let ((parent-dir
    ;;                      (file-name-directory (directory-file-name default-directory))))
    ;;            (setq args `(,consult-toggle-search-current-command ,parent-dir))))

    ;;         ('consult-git-grep
    ;;          (setq args '(consult-ripgrep))
    ;;          (when (not (string-match-p "^#" string))
    ;;            (setq string (consult--async-split-initial string))))

    ;;         ('consult-line-multi
    ;;          (setq args '(consult-ripgrep)
    ;;                string (consult--async-split-initial string)))

    ;;         ('consult-outline
    ;;          (setq args '(consult-line))))

    ;;       (when args
    ;;         (run-at-time 0 nil (lambda (comm)
    ;;                              ;; consult--customize-get 依赖 this-command
    ;;                              (setq this-command (car comm))
    ;;                              (setq consult-toggle-search-initial-string string)
    ;;                              (unwind-protect
    ;;                                  (apply comm)
    ;;                                (setq consult-toggle-search-initial-string nil
    ;;                                      consult-toggle-search-current-command nil)
    ;;                                (remove-hook 'pre-command-hook '+consult-delete-default-contents))) args)
    ;;         (minibuffer-quit-recursive-edit))
    ;;       ))
    ;;    (t
    ;;     (setq consult-toggle-search-current-command this-command)
    ;;     (if-let ((string consult-toggle-search-initial-string))
    ;;         (or (setq consult-toggle-search-initial-string nil) string)
    ;;       (setq string (cond
    ;;                     ((functionp action)
    ;;                      (funcall action))
    ;;                     ((stringp action)
    ...
    ;;                     ((null action) nil)
    ;;                     (t
    ;;                      (thing-at-point action t))))

    ;;       (cond ((and string default-text)
    ;;              (add-hook 'pre-command-hook '+consult-delete-default-contents)
    ;;              (let ((hook (make-symbol "+consult-goto-minibuffer-beg")))
    ;;                (fset hook (lambda ()
    ;;                             (remove-hook 'minibuffer-setup-hook hook)
    ;;                             (goto-char (if async
    ;;                                            (+ (length (consult--async-split-initial ""))
    ;;                                               (minibuffer-prompt-end))
    ;;                                          (minibuffer-prompt-end)))))
    ;;                (add-hook 'minibuffer-setup-hook hook))
    ;;              (if async
    ;;                  (consult--async-split-initial (propertize string 'face 'shadow))
    ;;                (propertize string 'face 'shadow)))
    ;;             (t
    ;;              (if async
    ;;                  (consult--async-split-initial string)
    ;;                string))))
    ;;     )))
    ...
    ;;            (define-key map (kbd "C-t") #'consult-toggle-search)
    ;;            map)
    ;;  consult-line
    ;;  consult-line-multi
    ;;  consult-outline
    ;;  :initial (consult-toggle-search t t)
    ;;  consult-imenu
    ;;  consult-imenu-multi
    ;;  :initial (consult-toggle-search)
    ;;  consult-find
    ;;  consult-ripgrep
    ;;  consult-grep
    ;;  consult-git-grep
    ;;  :initial (consult-toggle-search nil nil t)
    ;;  )
  ...
  (use-package perspective
    ...
    (progn
      ...
      (defvar lmm/perspective-consult--source-buffer
        (list :name     (format "Persp Buffers (%s)" (persp-current-name))
              :narrow   ?b
              :category 'buffer
